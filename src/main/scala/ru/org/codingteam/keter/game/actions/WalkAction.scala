  ...
  override def process(state: UniverseSnapshot, engine: IEngine) = {
    ...
    newPosition.surfaceAt match {
      case Some(s) if s.passable =>
        state
          .updatedActor(actor.id)(a => a.copy(position = newPosition))
          .updatedTimestamp(_ + duration)
      case _ =>
        state.updatedTimestamp(_ + duration)
    }
  }

  override def duration(state: UniverseSnapshot) = duration
}
